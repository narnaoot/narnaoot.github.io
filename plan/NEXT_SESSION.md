...
- **Name-tag preview = the real app screenshot** (`assets/img/Nametag.webp`), framed as a phone (rounded, bordered, drop-shadow, ~320px, centered) with an italic caption. This replaced an earlier HTML/SVG recreation — Nabil uploaded an actual screenshot (`assets/img/Nametag.png`, 1206×2622) and asked to use it instead, since the whole point is a photo tied to name + pronouns and the real avatars read better than any recreation. Converted PNG → resized WebP (760px wide, 652 KB → 56 KB, q82) to match the all-WebP convention; the `.png` was removed from the tree and remains in git history (recover at the "Add files via upload" commits). Verified in headless Chromium.
...
**Done:**
- **#9 — "is the job" (Probability teaser, Work).** ✅ Dropped the "…because making complex ideas accessible to decision-makers is the job." closer (mic-drop + corporate-speak echoing the retired "make complexity legible"). Teaser now ends on the actual hook: "Here's the intuition, without a single formula." Forced professional tie-back removed, same as Gardens. This closes out #9.
- **#9 — "a love letter to the city" / "is its own kind of" (Gardens).** ✅ Replaced the two clichés with the real story behind the project. The full anecdote now anchors the **Gardens page hero** (a coworker led Nabil into a Wells Fargo, past a security guard, up an elevator to a rooftop garden — "I've loved secrets like that ever since"; kicker: "Some even have bathrooms."). The **Work teaser** got a tighter hook (rooftop/office-tower gardens you reach "by walking past a security guard to an unmarked elevator"), and the Gardens meta description was rewritten to match. Dropped the forced "understanding a place is its own kind of project management" tie-back — this piece lives under "lighter reads" and doesn't need it.
- **#3 + #12 — eBay lead anaphora & buzzwords (Work).** ✅ Rewrote the "no shared map — no view of…, no way…, no operating rhythm that scaled" triple-"no" into three varied clauses ("The work in progress was invisible, product teams couldn't easily tell when they needed a security review, and the group's way of working didn't scale"). Kept the single "no shared map" — it pays off with "I built the map" two sentences later. Also swapped two buzzy phrases in the same paragraph: "unblocked the intake" → "smoothed out the intake", and "showed up as internal consultants" → "showed up as **partners, not obstacles**".
- **#11 — mic-drop closers (Work), part 1.** ✅ Dropped the flat closer "Each project tells a story." from the Work hero sub (Nabil wasn't attached to it). The iTunes case-study closer "It was a triumph." is the other #11 instance — not yet addressed.
- **Experience numbers corrected + DBA surfaced (Jul 16).** Nabil's actual tenure: ~25 years in tech — first five as a **DBA**, then two decades in project management / business analysis / project leadership. Updated Home: stats strip `19+ · Years experience` → `25+ · Years in tech`; hero body "Nineteen years of it" → "Over two decades of it" (that "it" = the building/leadership era, the post-DBA ~20 years); meta description rewritten ("Twenty-five years in tech: five as a DBA, then two decades in project leadership and business analysis, including seven at Apple…"). Added a DBA line to the About "Where I started" card ("My first job in tech was as a database administrator — a five-year stint before I moved into projects and analysis."), which also gives the "novels → databases → dashboards" arc a literal anchor. (Older `19+`/`Nineteen`-year references remain only in the `original_version/` and `pages_with_links/` archives.)
- **#7 — "0 → 1" / "Building what doesn't exist yet" repetition (Home hero area).** ✅ The motif appeared three times within ~60px at the top of Home. Kept **"0 → 1 is infinite"** as the H1 (its one earned home); rewrote the hero body ("…whatever the gap is, whatever's missing, that's what I build. Nineteen years turning what doesn't exist yet into what runs itself" → "…I build **the thing nobody's realized is missing**. Nineteen years of it: seven at Apple, now at SFMTA."); and reworded the "Favorite kind of problem" side-fact ("0 → 1. Building what doesn't exist yet." → "The blank-page kind — making something brand new."). Net: "0 → 1" now appears exactly once (headline), and "build what doesn't exist yet" drops to a single instance (Patterns toolkit).
- **#2 — stacked triads (Home "The work, briefly").** ✅ The Home about paragraph stacked three triads back-to-back (the novels→databases→dashboards arc, three matched "-ing" fragments, and the bold three-part signature). Rewrote the middle into flowing prose so only the bold signature triad remains; also swapped the third beat from "writing it down" to "figuring out how to explain it so the team can move" (Nabil's edit), trimmed "novels to databases to dashboards" → "novels to dashboards", and dropped the "same instinct" refrain (#5). Lone triads elsewhere (Work hero, About cats note, eBay step, toolkit column titles) left as-is — they read as voice, not tell. Also reworked the weak third beat of the **Patterns hero**: "…pick the right tool, know when I'm in the zone" → "…pick the right tool, and know when to push and when to leave room" (ties into the Bell Curve sweet-spot section and the whiteboard's cowboys→Big Brother "nudge the marker" idea).
- **#5/#6 — "make/translate complexity legible" repetition (Patterns + About).** ✅ Retired the abstract phrase by *showing* the idea instead. Added a new **"Everything Starts on a Whiteboard"** section to patterns.html (placed between Duct Tape/Drano and the Bell Curve, with a small `.wb` card style): whiteboard-first working habit, the "make it as simple as possible, but no simpler" rule, the Apple HA-backup sketch, and the cowboys→Big Brother spectrum ("I just want to put microtags on the sheep"). Reworded the About English-Lit portrait note off "make complexity legible" into a reading/writing framing ("…how writing carries an idea to a reader. I've been working on that same question ever since."). Left the Patterns toolkit grid untouched per decision (the "Making complexity legible" subtitle + "Translating Complexity" chip) — the new vignette now carries that concretely.
...
### Patterns — Duct Tape / Drano panel illustrations (Jul 16)
Added painted illustrations to the two "Two Kinds of Problems" panels: `duct_tape.webp` in the Duct Tape panel and `drano.webp` in the Drano panel. Title+sub are wrapped in a `.panel-head` flex with the image right (120px, stacks under the text below 560px). Recolored the Drano panel from teal to the site's pink — `.panel-drain` background → `--blush-lt`, its title and `.ptag-drain` pills → a new `--blush-dk #8E3455` token (AA on the blush tint and on cream); Duct Tape stays teal. The source PNGs were uploaded to master (commit `84ad87c`) at 1254×1254 / ~1.2 MB each with a checkerboard **baked in** (RGB, no alpha); converted to alpha WebP (~15 KB each) by flood-filling the baked background transparent + autocropping (Duct Tape needed an extra neutral-light key because the gray tape was close to the checker's lightness), matching the repo's WebP convention. The 1.2 MB PNGs were removed from the working tree (preserved in git history at `84ad87c`). Follow-up: re-cropped the Drano bottle tight to its content — the first pass left it centered in a full-frame 480×480 canvas, so at the same 120px box it looked dwarfed by the tape; now it fills its box and balances.

### Patterns — replaced the Bell Curve with "Microchipping Sheep" (Jul 16)
Nabil's call: the cowboys→Big Brother spectrum is the metaphor he actually uses for his work; the "My Professional Sweet Spot" bell curve was borrowed ("inspired by Catherine Madden"). So we dropped the bell curve entirely (SVG section + its `.bell-*` CSS) and gave the metaphor its own section, **"Microchipping Sheep"** (tag: "the metaphor I use"), in that slot — the whiteboard photo as centerpiece with his own words beneath it ("Every org I walk into sits somewhere on this line… I just want to microchip the sheep."). Note: the text now says **microchip** (matching the board), not the earlier "microtags". The "Everything Starts on a Whiteboard" vignette was trimmed to the habit + Einstein rule (the Apple HA-sketch detail folded into paragraph 2; the cowboys paragraph and the photo moved to the new section). Ripples: Home Patterns card reworded off "the sweet spot where I do my best work" → "how I think about giving an org just enough structure"; patterns.html meta description updated off the bell curve. New page flow: Favorite Questions → Two Kinds of Problems → Everything Starts on a Whiteboard → Microchipping Sheep → What I Bring.

### Patterns — whiteboard photo (Jul 16)
Added a photo of Nabil's actual cubicle whiteboard — the cowboys→"big brother" spectrum with "microchip the sheep" at the midpoint and a "POLICY" stack — directly under the "Everything Starts on a Whiteboard" vignette (the text describes the board; the photo shows it). `microchip.png` was uploaded to master (commit `35527fa`, 1536×1024, 1.9 MB); converted to `microchip.webp` (1400px, 68 KB) and framed with a `.wb-photo` figure (border + rounded corners + soft shadow) plus an italic "The board in my cubicle." caption. PNG removed from the working tree (kept in git history at `35527fa`).
