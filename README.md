...
| `eBaySecurityPainted.png` | ⏸ **needed** — work.html eBay Site Security case study, to remove visual asymmetry with iTunes/Kernel illustrations. Concept: see `plan/NEXT_SESSION.md` Image audit. |

## Still to do

- **eBay illustration** ⏸ — eBay Site Security is the only one of the three deep case studies without a painted illustration (iTunes has `QuietedPagersPainted.png`, Kernel has `KernelCleanupPainted.png`). Needs `eBaySecurityPainted.png` ("Map for the Maze" concept — see `plan/NEXT_SESSION.md` Image audit for full description) to remove visual asymmetry now that eBay is a peer of the other two case studies.
